...
//   { "proofHex": "0x...", "proofSizeBytes": N,
//     "proveMs": N, "verifyMs": N, "ethAddr": "0x..." }
...
// Ligero parameters — must match whatever the verifier uses.
static constexpr size_t kLigeroRate = 7;
static constexpr size_t kLigeroNreq = 132;
static constexpr size_t kVersion    = 7;
...
int main(int argc, char** argv) {
  ...
  if (!pkX_str || !pkY_str || !r_str || !s_str || !msg_str) {
    fprintf(stderr, "Usage: hidden_pk_prove "
  ...
  Transcript     tp(kLabel, kLabelLen, kVersion);
  ...
  ZkVerifier<Field, RSFactory> verifier(*CIRCUIT, rsf, kLigeroRate, kLigeroNreq,
                                        p256k1_base);
  Transcript tv(kLabel, kLabelLen, kVersion);
